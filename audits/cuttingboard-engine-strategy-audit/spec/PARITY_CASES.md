...
### Quoted from the literal-recovery amendment — DI-1, DI-2, DI-3

These three are no longer declared choices. Their literals are quoted from
[`TV-1-LITERAL-RECOVERY-AMENDMENT.md`](TV-1-LITERAL-RECOVERY-AMENDMENT.md), which transcribes
them from the pinned source for gate rows S-02, S-03/S-04 and D-02. Quoting the literal is not
a parity claim: the amendment authorizes no compilation, run, or parity acceptance, and
confirming the implementation against these literals remains TV-2's task.

| ID | Where | Frozen text | Authority literal, as implemented | Risk if wrong |
|---|---|---|---|---|
| DI-1 | S-02 REVERSAL | "absolute EMA9/21 spread `<0.2%`" — denominator unstated | Denominator is `ema21`. Stored signed as `(ema9 - ema21) / ema21`; `math.abs()` applied only in the REVERSAL comparison. Both comparisons strict | Denominator `close` instead of `ema21` shifts the REVERSAL boundary by roughly the price/EMA21 gap — small, but it changes label counts |
| DI-2 | S-03 / S-04 | "Determined from EMA alignment and price relative to EMA9/21"; CHOP is "degraded aligned structure, or fallthrough" | Bull aligned: `close >= ema9` TREND_UP, `ema21 <= close < ema9` PULLBACK_UP, `close < ema21` degraded → CHOP. Bear aligned: `close <= ema9` TREND_DOWN, `ema9 < close <= ema21` PULLBACK_DOWN, `close > ema21` degraded → CHOP. **Both boundary comparisons are inclusive** | Wrong band boundaries move mass between PULLBACK and CHOP, which changes Gate 4 rejection counts directly |
| DI-3 | D-02 ATR14 | "Pandas `ewm(alpha=1/14, adjust=False)` on true range" — true range and seed unstated | `TR = max(h-l, abs(h-c1), abs(l-c1))`, first bar seeded `h-l`, recursion seeded on the first TR to match `adjust=False` | A different seed changes early-history ATR, therefore stop distance, Gate 6 and Gate 10 in the warm-up window |

### Still declared — resolve these first

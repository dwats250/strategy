# PREMIUM CAMPAIGN MANIFEST — v2.4 (FROZEN) · 2026-07-22
v2.4 — methodological correction (audit round 6): sizing is path-
dependent, so exclusion-by-row was never quarantine. Changes:
(a) **R2 governing run starts 2016-01-01** — the emulator natively
begins with $25,000 in 2016, the paper's exact initial conditions. The
2010-start run is optional depth census only (bar arithmetic from the
two halts already shows QQQ 5m data reaches ~Jan 2010, so full-period
replication is data-feasible). (b) QQQ effective version **v1.1.7-rep**:
orphan sessions trade nothing new (close-and-reopen collision class
eliminated); closure bookkeeping loops over all newly registered
trades; exit classification via exit_comment(); cross-session trades
counted, with nOrphans an explicit lower bound — the authoritative
invalid set is determined offline from timestamps and comments.
(c) **Governing H2 equity path is computed OFFLINE, valid-only**:
rebuild per-trade equity_before from $25,000 + cumulative P&L, compound
valid trades only; report Σ|invalid P&L|/equity as the residual
contamination bound; TradingView's Performance Summary is treated as
contaminated whenever any invalid trade exists. (d) R2's export is
`deep_R2_QQQ_2016.csv`; the H2 slice is entries ≤ 2023-02-17 of THAT
run; post-paper is the remainder of the same run.
v2.3: QQQ effective version **v1.1.6-rep** — second execution-
discovered fix, completing v1.1.5's orphan quarantine (2018-07-05, the
session after the July-4th half-day: an immediate close does not
reflect in position state within its own execution; the stale position
collided with the new session's signal). Both deep halts to date map to
holiday half-day data holes (2013 Thanksgiving, 2018 July 4th) —
consistent with the ORPHAN-rare expectation. Nothing in the ordinary
entry/stop/target/EOD path has changed since v1.1.3.
...
| QQQ replication | `qqq_orb_replication_v1_1_7.pine` | **v1.1.7-rep** |
...
| R2 | QQQ 5m, ETH off | v1.1.7 · frozen params (1%/10R/4×) | **2016-01-01 → today** | `deep_R2_QQQ_2016.csv` |
